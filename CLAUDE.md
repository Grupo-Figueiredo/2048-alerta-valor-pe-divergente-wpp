...
```mermaid
flowchart LR
    feat["feat/* · fix/* · chore/* · docs/* ..."] -->|PR| dev
    dev -->|checkout, sem PR| release["release/* (efêmera)"]
    release -->|PR, code owner| main
    hotfix["fix/hotfix-* (a partir da main)"] -->|PR| main
    main -.->|back-merge automático| dev
```
...
```mermaid
flowchart TD
    main["main.py"] --> controladores["aplicacao/controladores"]
    controladores --> servicos["aplicacao/servicos"]
    servicos --> repositorios["repositorios/ (API V2 · /v2/query/)"]
    repositorios --> utils["utils/ (ClienteApiV2, Logger, Credenciais, Arquivos)"]
    controladores -.-> excecoes["aplicacao/excecoes (ExcecaoNegocio)"]
```
